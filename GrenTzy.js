...
bot.onText(/\/checkupdate/, async (msg) => {
  ...
  try {
    ...
    const normRemote = normalize(remoteData);
    const normLocal = normalize(localData);
    ...
    if (normRemote === normLocal) {
      ...
      const charDiff = Math.abs(remoteData.length - localData.length);
      const percentDiff = ((charDiff / localData.length) * 100).toFixed(2);
      ...
      await bot.editMessageText(
        ...
        `📊 Perbedaan   : ${charDiff} karakter (${percentDiff}%)`,
        {
          chat_id: chatId,
          message_id: statusMsg.message_id,
          parse_mode: "Markdown"
...
bot.onText(/\/checkfile(?:\s+(\S+))?/, async (msg, match) => {
  ...
  if (!isOwner(senderId)) {
    return bot.sendMessage(chatId, "❌ Hanya owner yang bisa mengecek file.");
  }

  const filename = match[1];
  if (!filename) {
    return bot.sendMessage(chatId, "❌ Gunakan: /checkfile <nama_file>\nContoh: /checkfile GrenTzy.js");
  }

  const repoOwner = "khususbanding749-ai";
  const repoName = "GrenTzy1";
  const branch = "main";
  const path = filename; // asumsi file di root repo

  const apiUrl = `https://api.github.com/repos/${repoOwner}/${repoName}/contents/${encodeURIComponent(path)}?ref=${branch}`;
  ...
  try {
    ...
    if (Array.isArray(data)) {
      // Jika path adalah folder
      const fileNames = data.map(item => item.name).join(', ');
      return bot.sendMessage(chatId, `📁 *${path}* adalah folder.\nIsi: ${fileNames}`, { parse_mode: 'Markdown' });
    }
    ...
    const htmlUrl = fileInfo.html_url || `https://github.com/${repoOwner}/${repoName}/blob/${branch}/${path}`;
    const fileName = fileInfo.name;

    let message = `📄 *File: ${fileName}*\n`;
    message += `📦 Ukuran: ${(fileSize / 1024).toFixed(2)} KB (${fileSize} bytes)\n`;
    message += `🔑 SHA: \`${fileSha}\`\n`;
    message += `🔗 [Download](${downloadUrl})\n`;
    message += `🌐 [Lihat di GitHub](${htmlUrl})\n`;
    message += `📂 Repo: ${repoOwner}/${repoName}\n`;
    ...
    if (error.response && error.response.status === 404) {
      return bot.sendMessage(chatId, `❌ File \`${filename}\` tidak ditemukan di repository.`, { parse_mode: 'Markdown' });
    }
...
bot.onText(/^\/listrepo(?:\s+(\S+))?$/, async (msg, match) => {
  ...
  try {
    ...
    if (!Array.isArray(data)) {
      const fileInfo = data;
      return bot.sendMessage(chatId,
        `📄 <b>${fileInfo.name}</b> adalah file, bukan folder.\nGunakan /checkfile ${fileInfo.name} untuk detail.`,
        { parse_mode: 'HTML' }
      );
    }
    ...
    if (data.length === 0) {
      return bot.sendMessage(chatId,
        `📁 Folder <code>${subPath || '/'}</code> kosong.`,
        { parse_mode: 'HTML' }
      );
    }
    ...
    let message = `<b>📂 Repository: ${repoOwner}/${repoName}</b>\n`;
    message += `📁 Path: <code>${subPath || '/'}</code>\n`;
    message += `🌿 Branch: ${branch}\n`;
    message += `━━━━━━━━━━━━━━━━━━━━━━\n`;

    if (folders.length > 0) {
      message += `<b>📁 FOLDER (${folders.length})</b>\n${folders.join('\n')}\n\n`;
    }
    if (files.length > 0) {
      message += `<b>📄 FILE (${files.length})</b>\n${files.join('\n')}\n`;
    }
    ...
    if (error.response && error.response.status === 404) {
      return bot.sendMessage(chatId,
        `❌ Path <code>${subPath || '/'}</code> tidak ditemukan.`,
        { parse_mode: 'HTML' }
      );
    }
